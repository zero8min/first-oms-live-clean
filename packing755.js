...
window.renderJob=function(){
 ...
 app.innerHTML=`<div class="top"><div class="worker">${E(worker)}님 <span class="pill">체크 자동저장</span></div><button class="change" onclick="changeWorker()">포장자 변경</button></div><div class="job">${E(job.jobNo||'')}번 / ${E(job.name||job.nick||'고객')}</div><div class="count">📦 실제 포장 총 ${totalQty(job.items)}단위</div><div class="items">${rows}</div>${anyShortage()?'<div class="notice warn"><b>재고부족 상품이 있습니다.</b><br>관리자 택배실에도 표시됩니다. 부족건을 해결한 뒤 체크해 주세요.</div>':''}<button class="btn good" ${allResolved()?'':'disabled style="opacity:.45"'} onclick="packed()">${allResolved()?'다 넣었어요':'모든 상품을 체크해주세요'}</button><div class="small">사이즈·색상·향·수량별로 확인 후 체크하세요. 재고가 없으면 해당 행의 재고부족 버튼을 누르세요.</div>`;
};
